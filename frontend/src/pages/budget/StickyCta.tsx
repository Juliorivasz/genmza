...
export function StickyCta({ onClick, disabled = false, ready = false }: StickyCtaProps) {
  return (
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden px-4 pb-safe-bottom pb-6 pt-4 bg-gradient-to-t from-gray-950 via-gray-950/95 to-transparent">
      <button
        type="button"
        onClick={onClick}
        disabled={disabled}
        className={[
          ...
          ready
            ...
            : 'bg-white/10 text-gray-400 border border-white/10 focus-visible:ring-cyan-400',
        ].join(' ')}
      >
        <div className="flex items-center gap-3">
          <MessageCircle size={22} className={ready ? 'text-white' : 'text-gray-500'} />
          <div className="text-left">
            <div className="text-base font-bold leading-none">
              Consultar por WhatsApp
            </div>
            <div className={['mt-0.5 text-xs font-normal leading-none', ready ? 'text-emerald-100' : 'text-gray-600'].join(' ')}>
              {ready ? '¡Listo para enviar!' : 'Completá el formulario para continuar'}
            </div>
          </div>
        </div>
        <ChevronRight size={20} className={ready ? 'text-white' : 'text-gray-600'} />
      </button>
    </div>
  );
}
